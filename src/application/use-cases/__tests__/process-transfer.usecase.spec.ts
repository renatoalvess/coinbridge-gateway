...
describe('ProcessTransferUseCase', () => {
  ...
  beforeEach(() => {
    ...
    exchangeRateProviderMock = {
      getRate: vi.fn().mockResolvedValue(5.5), // 1 USDT = 5.50 BRL
    };
  ...
  it('should process transfer successfully with exchange rate applied', async () => {
    let saveCount = 0;
    transactionRepositoryMock.save = vi
      .fn()
    ...
    expect(exchangeRateProviderMock.getRate).toHaveBeenCalledWith(
      'USDT',
      'BRL',
    );
    // amount is 10000, rate is 5.50 -> amountBrl = 55000
    expect(pixProviderMock.sendPix).toHaveBeenCalledWith(55000, 'merchant-123');
    expect(queueProviderMock.publishWebhookEvent).toHaveBeenCalledWith(
      'tx-internal-123',
    );
    expect(saveCount).toBe(2);
    expect(mockTransaction.exchangeRate).toBe(5.5);
  ...
  it('should throw an error if transaction is not found', async () => {
    ...
    await expect(processTransferUseCase.execute('invalid-id')).rejects.toThrow(
  ...
  it('should mark transaction as FAILED if Pix provider returns false', async () => {
    ...
    expect(transactionRepositoryMock.save).toHaveBeenLastCalledWith(
  ...
  it('should mark transaction as FAILED and rethrow if Pix provider throws an error', async () => {
    ...
    await expect(
      processTransferUseCase.execute('tx-internal-123'),
